...
async function setupIframe(el, polite, r1, g1, b1, r2, g2, b2) {
  el.srcdoc = `<!DOCTYPE html>
      <html>
      <body>
      <h3 style="font-family: 'Roboto', sans-serif; font-weight: 400;">${
        polite ? "Polite" : "Impolite"
      } Peer's iframe</h3>
      <div id="buttons">
        <button onclick="(${startLocalVideo.toString()})(${r1},${g1},${b1},${r2},${g2},${b2});" id="${
    polite ? "politeStart" : "impoliteStart"
  }">Start</button>
      ...
      <script>
        (${peer.toString()})(window.parent.document.getElementById("${
    polite ? "impolite" : "polite"
  }").contentWindow, ${polite});
      </script>
      <html>`;
