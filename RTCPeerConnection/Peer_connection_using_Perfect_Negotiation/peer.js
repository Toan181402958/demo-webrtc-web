...
function peer(other, polite, fail = undefined) {
  ...
  if (!fail)
    fail = (e) =>
      void send(window.parent, { error: `${e.name}: ${e.message}` });
  const send = (target, msg) =>
    void target.postMessage(JSON.parse(JSON.stringify(msg)), "*");
  const log = (str) =>
    void console.log(`[${polite ? "POLITE" : "IMPOLITE"}] ${str}`);
  const assert_equals = !window.assert_equals
    ? (a, b, msg) =>
        a === b || void fail(new Error(`${msg} expected ${b} but got ${a}`))
